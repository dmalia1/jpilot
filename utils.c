...
int cal_dialog(GtkWindow *main_window,
    ...
    g_signal_connect(G_OBJECT(cal), "day_selected_double_click", G_CALLBACK(cb_quit),
...
char *charset_p2newj(const char *buf, int max_len, int char_set) {
    ...
    * Only old conversion routines don't assign a buffer */
    ...
    switch (char_set) {
        case CHAR_SET_JAPANESE :
            Sjis2Euc(newbuf, max_len);
            break;
        case CHAR_SET_LATIN1   : /* No conversion required */ break;
        case CHAR_SET_1250     :
            Win2Lat(newbuf, max_len);
            break;
        case CHAR_SET_1251     :
...
#include "icons/list_mini_icons.h"
    ...
    switch (which_pixbuf) {
        ...
        case PIXMAP_BOX_CHECK:
            *out_pixbuf = pixbuf_check;

            break;
        case PIXMAP_BOX_CHECKED:
            *out_pixbuf = pixbuf_checked;
            break;
        case PIXMAP_FLOAT_CHECK:
            *out_pixbuf = pixbuf_float_check;
            break;
        case PIXMAP_FLOAT_CHECKED:
            *out_pixbuf = pixbuf_float_checked;
            break;
        case PIXMAP_SDCARD:
            *out_pixbuf = pixbuf_sdcard;
            break;
        default:
            *out_pixbuf = NULL;
    }
