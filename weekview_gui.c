...
void cb_weekview_quit(GtkWidget *widget, gpointer data)
{
   ...
   gdk_window_get_size(gtk_widget_get_window(weekview_window), &w, &h);
...
void weekview_gui(struct tm *date_in)
{
   ...
   gtk_signal_connect(G_OBJECT(weekview_window), "destroy",
   ...
   gtk_signal_connect(G_OBJECT(weekview_window), "delete_event",
                      G_CALLBACK(cb_weekview_quit), NULL);
   ...
   for (i=0; i<8; i++) {
      ...
      gtk_signal_connect(G_OBJECT(week_day_text[i]), "button_release_event",
