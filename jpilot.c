...
void call_plugin_gui(int number, int unique_id) {
    ...
    if (glob_date_timer_tag) {
        g_timeout_remove(glob_date_timer_tag);
    }
...
static void cb_restore(GtkWidget *widget, gpointer data) {
    ...
    get_window_size(gtk_widget_get_window(window), &w, &h);
...
static void cb_delete_event(GtkWidget *widget, GdkEvent *event, gpointer data) {
    ...
    get_window_size(gtk_widget_get_window(window), &pw, &ph);
...
#  endif
    ...
    /* Set the pane size */
    width = height = 0;
    get_window_size(gtk_widget_get_window(window), &width, &height);
