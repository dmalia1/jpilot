    ...
    if ((con_or_dis == CONNECT_SIGNALS) && (!connected)) {
        ...
        for (i = 0; i < NUM_KEYRING_CAT_ITEMS; i++) {
            if (keyr_cat_menu_item2[i] != NULL) {
                gtk_signal_connect(G_OBJECT(keyr_cat_menu_item2[i]),
                                   "toggled",
        ...
        gtk_signal_connect(G_OBJECT(entry_name), "changed",
                           G_CALLBACK(cb_record_changed), NULL);
        gtk_signal_connect(G_OBJECT(entry_account), "changed",
                           G_CALLBACK(cb_record_changed), NULL);
        gtk_signal_connect(G_OBJECT(entry_password), "changed",
                           G_CALLBACK(cb_record_changed),
                           GINT_TO_POINTER(PASSWD_FLAG));
        gtk_signal_connect(G_OBJECT(date_button), "pressed",
                           G_CALLBACK(cb_record_changed), NULL);
    ...
    if ((con_or_dis == DISCONNECT_SIGNALS) && (connected)) {
        ...
        for (i = 0; i < NUM_KEYRING_CAT_ITEMS; i++) {
            if (keyr_cat_menu_item2[i]) {
                gtk_signal_disconnect_by_func(G_OBJECT(keyr_cat_menu_item2[i]),
                                              G_CALLBACK(cb_record_changed),
                                              NULL);
            }
        }
...
int plugin_gui(GtkWidget *vbox, GtkWidget *hbox, unsigned int unique_id) {
    ...
    gtk_signal_connect(G_OBJECT(date_button), "clicked",
