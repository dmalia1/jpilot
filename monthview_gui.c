   ...
   for (i=0; i<6; i++) {
      ...
      for (j=0; j<7; j++) {
         ...
         if (n<37) {
            ...
            gtk_signal_connect(G_OBJECT(event_box), "enter_notify_event",
                               G_CALLBACK(cb_enter_notify), GINT_TO_POINTER(n));
            gtk_signal_connect(G_OBJECT(text), "button_release_event",
...
void monthview_gui(struct tm *date_in)
{
   ...
   gtk_signal_connect(G_OBJECT(monthview_window), "destroy",
   ...
   gtk_signal_connect(G_OBJECT(monthview_window), "delete_event",
