...
static void connect_changed_signals(int con_or_dis) {
    ...
    if ((con_or_dis == DISCONNECT_SIGNALS) && (connected)) {
        ...
        if (datebook_version) {
            for (i = 0; i < NUM_DATEBOOK_CAT_ITEMS; i++) {
                if (dbook_cat_menu_item2[i]) {
                    g_signal_handlers_disconnect_by_func(G_OBJECT(dbook_cat_menu_item2[i]),
                                                  G_CALLBACK(cb_record_changed), NULL);
                }
            }
        }
...
int datebook_gui(GtkWidget *vbox, GtkWidget *hbox) {
   ...
   g_signal_connect(G_OBJECT(radio_button_appt_time), "clicked",
    ...
    if (use_db3_tags) {
        ...
        datebk_entry = new_gtk_entry_with_max_length(30);
    ...
    repeat_day_entry = new_gtk_entry_with_max_length(2);
    ...
    repeat_week_entry = new_gtk_entry_with_max_length(2);
    ...
    repeat_mon_entry = new_gtk_entry_with_max_length(2);
    ...
    repeat_year_entry = new_gtk_entry_with_max_length(2);
